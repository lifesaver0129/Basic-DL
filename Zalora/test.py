...
'''
import requests,re,time,os,gc,pymysql,sys,io
...
def saveImgs(img_path):
    img_list = driver_1.find_elements_by_xpath('//ul[@class="prd-moreImagesList ui-listItemBorder ui-listLight swiper-wrapper"]/li')
    img_url_list = []
...
def crawlPageContent(driver):
    ...
    product_details_data = (product_breadcrumbs, product_sku, product_website, 
                            ...
                            img_number, product_img_path, product_reviews, start_number)
    cursor_1.execute(sql_update_content, product_details_data)
    db_1.commit()#需要这一句才能保存到数据库中

#begin the main function
#connect the database.
sys.stdout = io.TextIOWrapper(sys.stdout.buffer,encoding='gb18030')#改变标准输出的默认编码  

db_1 = pymysql.connect("localhost","user","123456","testdb", charset="utf8")
# 使用cursor()方法获取操作游标 
...
while start_number > 50000:
    # SQL 查询语句
    sql_fetch_product_url_by_id_1 = "SELECT product.product_URL FROM testdb.product where product.idproduct = %s"
    cursor_1.execute(sql_fetch_product_url_by_id_1, start_number)
    fetch_product_url_by_id_category_results_1 = cursor_1.fetchone()
    product_url_1 = fetch_product_url_by_id_category_results_1[0]
    #end of fetch product_url
    
    #start open and load a product URL
    #time_start = time.time()
    firefoxProfile_1 = webdriver.FirefoxProfile()
    driver_1 = webdriver.Firefox(firefox_profile = firefoxProfile_1)
    #driver = webdriver.PhantomJS('phantomjs')
    #handle_now = driver.current_window_handle
    #print('Time consuming 1:', time.time() - time_start)
    driver_1.get(product_url_1)
