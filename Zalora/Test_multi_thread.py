...
class myThread (threading.Thread):
    ...
    def run(self):
        # 获取锁，用于线程同步
        threadLock.acquire()
        crawling(self.startnumber, self.endnumber)
        # 释放锁，开启下一个线程
        threadLock.release()
...
def crawlPageContent(driver, start_number_one):
    ...
    img_number = saveImgs(driver, product_path)
...
def crawling(startnumber, endnumber):
    start_number_one = startnumber
    while start_number_one < endnumber:
        # SQL 查询语句
        ...
        #******************************************************'''
        try:
            crawlPageContent(driver, start_number_one)
        except NoSuchElementException as e:
...
# 创建新线程
thread1 = myThread(1, "Thread-1", 28126, 35000)
thread2 = myThread(2, "Thread-2", 35000, 50000)
