...
def find_feature_match_productList(productID, category_match_pool, assignedNumber, feature_matchList, feature_scoreList):
    assignedNumber = round(assignedNumber)
    outputList = []
    scoreList = []
    for i in range(len(category_match_pool)):
...
def find_product_match_score(productInfoList, category_match_product_poolList, feature_matchList, feature_scoreList):
    ...
    for i in range(len(category_match_product_poolList)):
        ...
        ele_category_scoreList = calculate_match_score_in_category(productInfoList, category_match_product_poolList[i], feature_matchList, feature_scoreList)
...
def calculate_match_score_for_each_product(productInfoList, ele, feature_matchList, feature_scoreList):
    ...
    if float(match_product_info[5]) > float(productInfoList[5])*0.8 and float(match_product_info[5]) < float(productInfoList[5])*1.2:
        price_score = 1
...
#         productID = 5283
        ...
        if product_category == '': # there is no match in category level, no match in the last.
        ...
        if len(category_matchList) == 0: # there is no match in category level, no match in the last.
        ...
        else:# the category_matchList is not null, exits at least one match category.
            ...
            category_match_score_poolList = find_product_match_score(productInfoList, category_match_product_poolList, feature_matchList, feature_scoreList)
