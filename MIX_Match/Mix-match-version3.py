...
def find_category_level_match(target_product_category_code):
    ...
    sqlString = 'SELECT matchcategorycode, score FROM clean_data_zalora.mix_match_pairs where categorycode = %s and level = %s and matchcategorycode like %s'
    cursor.execute(sqlString, (target_product_category_code, '2', '%P%')) # first level category match
...
def find_feature_level_match(target_product_category):
    ...
    if len(target_product_category) > 0:
        for ele in target_product_category:
...
def find_products_with_featureLevel(categoryCode):
    productList = []
    
    if len(feature_matchList) > 0:
        for ele_feature_matchList in feature_matchList:
            sqlString = "SELECT idproduct from clean_data_zalora.product where category = %s and feature != ''"
            cursor.execute(sqlString, categoryCode)
            fetch_result = cursor.fetchall()
