...
def find_products_with_featureLevel(categoryCode, feature_under_the_category):
    ...
    if len(feature_under_the_category) > 0:
        for ele_feature_matchList in feature_under_the_category:
            sqlString = "SELECT idproduct from clean_data_zalora.product where category = %s and feature like %s and (colour = %s or brand = %s)"
            data = (
            categoryCode, '%' + ele_feature_matchList + '%', target_productInfoList[7], target_productInfoList[3])
...
def find_products_with_no_featureLevel(categorylevelcode):
    outputList = []
    sqlString = "SELECT idproduct from clean_data_zalora.product where category = %s and feature = '' and (colour = %s or brand = %s)"
    data = (categorylevelcode, target_productInfoList[7], target_productInfoList[3])
    cursor.execute(sqlString, data)
    fetch_result = cursor.fetchall()
...
if __name__ == '__main__':
    ...
    for ele_zaloraProducts in zaloraProducts:  # Define, Call this productID as target product.
        ...
        target_product_featureList = target_productInfoList[9].split(
        ...
        if len(feature_matchList) > 0:  # there are feature level matching
            ...
            diff_feature_matchList_parent = list(
                set(category_matchList) - set(feature_matchList_parent))  # category_matchList - featurelist's parent.
