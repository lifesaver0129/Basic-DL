...
def find_products_with_featureLevel(categoryCode, feature_under_the_category):
    ...
    if len(feature_under_the_category) > 0:
        for ele_feature_under_the_category in feature_under_the_category:
            sqlString = "SELECT idproduct from clean_data_zalora.product where category = %s and feature like %s and (colourNo = %s or brand = %s)"
            data = (categoryCode, '%' + ele_feature_under_the_category + '%', target_productInfoList[8],
                    target_productInfoList[3])
...
def find_products_with_no_featureLevel(categorylevelcode):
    outputList = []
    sqlString = "SELECT idproduct from clean_data_zalora.product where category = %s and feature = '' and (colourNo = %s or brand = %s)"
    data = (categorylevelcode, target_productInfoList[8], target_productInfoList[3])
    cursor.execute(sqlString, data)
    fetch_result = cursor.fetchall()
...
def calculate_product_match_score(target_productInfoList, category_match_product_poolList, feature_matchList,
                                  feature_scoreList):
    product_matchList = []
    for i in range(len(category_match_product_poolList)):
        ele_category_scoreList = []
        ele_category_scoreList = calculate_match_score_in_each_category(target_productInfoList,
                                                                        category_match_product_poolList[i],
...
def calculate_match_score_in_each_category(target_productInfoList, product_list, feature_matchList, feature_scoreList):
    ...
    if len(product_list) > 0:
        for ele_product_list in product_list:
            matchScore = calculate_match_score_for_each_product(target_productInfoList, ele_product_list,
                                                                feature_matchList, feature_scoreList)
...
def calculate_match_score_for_each_product(target_productInfoList, productID, feature_matchList, feature_scoreList):
    ...
    if colour_similarity(target_productInfoList[8], match_product_info[8]) == 1:
        color_score = 1
    ...
    if price_level_target == 0 or price_level_match == 0:
        price_score = 1 - abs(float(match_product_info[5]) - float(target_productInfoList[5])) / float(
            target_productInfoList[5])
    ...
    match_product_featureList = match_product_info[10].split(';')
...
def update_info_to_website(productID, productInfo):
    db2 = pymysql.connect("158.132.122.211", "root", "tozmartdev2016", "b2c-zw", 13306, charset="utf8")
    cursor2 = db2.cursor()
...
if __name__ == '__main__':
    ...
    db = pymysql.connect(host="localhost",
                         ...
                         charset="utf8")
    cursor = db.cursor()
    zaloraProducts = open("C:/Users/Administrator/Desktop/zalora.txt", encoding='utf8').readlines()
    ...
    for ele_zaloraProducts in zaloraProducts:  # Define, Call this productID as target product.
        ...
        target_product_category = target_productInfoList[9]
        target_product_featureList = target_productInfoList[10].split(
            ';')  # given a product, features selected under the category.
        ...
        if len(feature_matchList) > 0:  # there are feature level matching
            ...
            diff_feature_matchList_parent = list(set(category_matchList) - set(
                feature_matchList_parent))  # category_matchList minus(-) featurelist's parent.
            ...
            if len(feature_matchList_parent) > 0:
                for ele_category_matchList in feature_matchList_parent:  # find products pool in each match category.
                    ...
                    ele_match_productList = find_products_with_featureLevel(ele_category_matchList,
                                                                            feature_under_the_category)
                    category_match_product_poolList.append(ele_match_productList)
        ...
        category_match_score_poolList = calculate_product_match_score(target_productInfoList,
                                                                      category_match_product_poolList,
                                                                      feature_matchList, feature_scoreList)
